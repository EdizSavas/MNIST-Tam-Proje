...
if __name__ == "__main__":
    ...
    if os.path.exists(custom_dir):
        ...
        if len(custom_imgs) >= 300:
            ...
            custom_imgs = custom_imgs.astype(np.float32)
            custom_imgs = (custom_imgs - 127.5) / 127.5
            custom_imgs = custom_imgs.reshape(len(custom_imgs), 784)
    ...
    test_data = (test_images.astype(np.float32) - 127.5) / 127.5
    test_data = test_data.reshape(10000, 784)
    test_output = myNeuralNet.forward(test_data)
    test_accuracy = np.mean(np.argmax(test_output, axis=1) == test_labels)
    print(f"[+] Deneme {trial+1} tamamlandı → Test Doğruluğu: {test_accuracy:.4f}")
    
    if test_accuracy > best_acc:
        best_acc = test_accuracy
        best_loss = losses[-1]
        best_weights = [w.copy() for w in myNeuralNet.weights]
        best_biases = [b.copy() for b in myNeuralNet.biases]

        print(f"[+] Bu model şu anki en iyisi!")
        
    if best_weights is not None and best_biases is not None:
        best_model_dir = os.path.join(base_dir, "best_model")
        os.makedirs(best_model_dir, exist_ok=True)
        for i, w in enumerate(best_weights):
            np.save(os.path.join(best_model_dir, f"final_weight_{i}.npy"), w)
        for i, b in enumerate(best_biases):
            np.save(os.path.join(best_model_dir, f"final_bias_{i}.npy"), b)

        print(f"\n[+] EN İYİ MODEL KAYDEDİLDİ → Doğruluk: {best_acc:.4f}, Loss: {best_loss:.4f}")
